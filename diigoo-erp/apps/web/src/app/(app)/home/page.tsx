...
export default function HomePage() {
  ...
  return (
    <>
      <div className="relative mb-4 overflow-hidden rounded-lg border border-line bg-surface p-4 shadow-card">
        ...
        <div className="relative">
          ...
          <h1 className="mt-1 text-xl font-bold text-navy">Welcome, {firstName}</h1>
          <p className="mt-0.5 text-sm text-ink-3">{role.label} · {session.tenant.branding.companyName} · {mods.length} module{mods.length === 1 ? "" : "s"} available</p>
        </div>
      </div>
        ...
        {mods.map((m) => {
          ...
          return (
            <Link
              key={m.key}
              href={m.basePath}
              className="group flex items-start gap-3 rounded-lg border border-line bg-surface p-3.5 shadow-card transition-all hover:-translate-y-0.5 hover:border-orange/40 hover:shadow-pop"
            >
              <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-white shadow-sm ring-1 ring-black/[.06]">
                <ModuleIcon moduleKey={m.key} size={26} fallback={m.icon} />
              </span>
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5">
                  <span className="truncate text-sm font-bold text-navy group-hover:text-orange">{m.name}</span>
                  <span className={`shrink-0 rounded px-1 py-0.5 text-[8px] font-bold uppercase tracking-wide ${live ? "bg-green/12 text-green" : "bg-ink-3/12 text-ink-3"}`}>{live ? "Live" : "Preview"}</span>
                </div>
                <p className="mt-0.5 line-clamp-2 text-[11px] leading-snug text-ink-3">{desc}</p>
              </div>
              <ArrowRight size={14} className="mt-0.5 shrink-0 text-line transition-transform group-hover:translate-x-0.5 group-hover:text-orange" />
            </Link>
          );
        })}
      </div>
      ...
      {!isEmployee && platform.length > 0 && (
        <>
          <div className="mt-5"><SectionLabel>Platform</SectionLabel></div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {platform.map((p) => (
              <Link key={p.href} href={p.href} className="group flex items-center gap-3 rounded-md border border-line bg-surface px-3 py-2.5 shadow-card transition-all hover:border-orange/40 hover:shadow-pop">
                <span className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-navy/[.06] text-navy"><p.icon size={16} /></span>
